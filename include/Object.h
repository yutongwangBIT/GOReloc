...
class Object
{
    public:
        ...
        Object(const Ellipsoid& ell, unsigned int id, std::unordered_map<int, int> cat_ids){ //FOR SAVE AND LOAD
            ellipsoid_ = ell;
            id_ = id;
            categories_with_counts = cat_ids;
            maxCountCat = 0;
            for(auto [cat_id, count] : cat_ids){
        ...
        std::unordered_map<int, int> GetAllCategoryIds() const {
            return categories_with_counts;
        }

        void AddCategoryId(int cat){ //CAN BE PRIVATE
            categories_with_counts[cat]++;
            if(categories_with_counts[cat]>maxCountCat){
                maxCountCat = categories_with_counts[cat];
                category_id_ = cat;
            }
        }
        ...
        void SetFlagOptimized(bool bflag){
            flag_optimized = bflag;
        }
    ...
    public:
        ...
        int linewidth = 2; //TMP TOBE DELETED

    protected:
        unsigned int category_id_; 
        std::unordered_map<int, int> categories_with_counts;
        int maxCountCat;
        unsigned int id_;
