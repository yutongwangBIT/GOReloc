...
public:
    ...
    void computeError()  {
        ...
        _error[0] = 1 - normalized_gaussian_wasserstein_2d(ellipse_, proj, 100);
        //std::cout << "wasser==>" << _error[0] << "\n";
    }
...
class  EdgeSE3ProjectEllipsoidOnlyPoseAndDepth: public g2o::BaseUnaryEdge<1, double, g2o::VertexSE3Expmap>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeSE3ProjectEllipsoidOnlyPoseAndDepth(const double& depth, const Ellipse& ell2d, const Ellipsoid& ell3d, const Eigen::Matrix3d& K) : depth_(depth), ellipse_(ell2d), ellipsoid_(ell3d), K_(K) {}
    ...
    void computeError()  {
        ...
        P << R, t;
        P = K_ * P;*/
        Eigen::Vector3d center_3d = ellipsoid_.GetCenter();
        //double z = R.row(2).dot(center_3d) + t[2];
        //_error[0] = z - depth_;

        Eigen::Vector2d bb_center = ellipse_.GetCenter();
        double u = (bb_center(0) - K_(0, 2)) / K_(0, 0);
        double v = (bb_center(1) - K_(1, 2)) / K_(1, 1);
        Eigen::Vector3d bb_center_cam(u*depth_, v*depth_, depth_);
        Eigen::Vector3d center_world = R.transpose() * bb_center_cam + (-R.transpose() * t);

        _error[0] = (center_3d - center_world).norm();
        //std::cout << "depth==>" << _error[0] << "\n";
    }
...
class  EdgeSE3ProjectPnP: public g2o::BaseUnaryEdge<2, Eigen::Vector2d, g2o::VertexSE3Expmap>{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    EdgeSE3ProjectPnP(const Eigen::Vector3d& p_3d, const Eigen::Vector2d& p_2d, const Eigen::Matrix3d& K) : p3d(p_3d), p2d(p_2d), K_(K) {}
    ...
    void computeError()  {
        ...
        Eigen::Vector3d p2d_est = K_ * (R * p3d + t);
        _error[0] = (p2d[0] - p2d_est[0] / p2d_est[2]) * 1.0;
        _error[1] = (p2d[1] - p2d_est[1] / p2d_est[2]) * 1.0;
        //std::cout << "error = " << _error[0] << " " << _error[1] << "\n";
    }
