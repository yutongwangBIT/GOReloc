...
void MapDrawer::DrawMapPoints(double size, bool ignore_objects_points)
{
    ...
    cv::Mat tmp_pos = mpMap->GetTmpPointsPos();
    ...
    for(int i = 0; i<tmp_pos.rows; i++)
    {
        cv::Mat pos = tmp_pos.row(i);
        glVertex3f(pos.at<float>(0), pos.at<float>(1), pos.at<float>(2));
    }
...
void MapDrawer::DrawMapObjectsPoints(double size)
{
    ...
    for (auto* obj : objects) {
        ...
        auto assoc_points = obj->GetAssociatedMapPoints();
        glPointSize(size);
        glBegin(GL_POINTS);
...
void MapDrawer::DrawCurrentCamera(pangolin::OpenGlMatrix &Twc)
{
    ...
    const float h = w*1.25;//w*0.75;
    const float z = w*1.0;//w*0.6;
...
void MapDrawer::DrawMapObjects()
{
    ...
    /*if (mpMap->graph_3d){
        std::map<int, Object*> m_map_objects_by_id;
        for (auto *obj : objects) {
            auto obj_id = obj->GetId();
            m_map_objects_by_id[obj_id] = obj;
        }
        for (auto *obj : objects) {
            auto obj_id = obj->GetId();
            if(mpMap->graph_3d->has_node(obj_id)){
                auto neighbours = mpMap->graph_3d->nodes[obj_id];
                for(auto n_id : neighbours){
                    if(mpMap->graph_3d->has_node(n_id)){
                        //std::cout<<"mapdrawer link:"<<obj_id<<", "<<n_id<<std::endl;
                        auto neighbour_obj = m_map_objects_by_id[n_id];
    ...
    for (auto *obj : objects) {
        ...
        //Eigen::Vector3d cen = ell.GetCenter();
