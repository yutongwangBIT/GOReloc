...
cv::Mat FrameDrawer::DrawFrame()
{
    ...
    /*if(flag_reloc){
        const float r0 = 5;
        const int n0 = vCurrentKeys.size();
        for(int i=0;i<n0;i++)
        {
            cv::Point2f pt1,pt2;
            pt1.x=vCurrentKeys[i].pt.x-r0;
            pt1.y=vCurrentKeys[i].pt.y-r0;
            pt2.x=vCurrentKeys[i].pt.x+r0;
            pt2.y=vCurrentKeys[i].pt.y+r0;
